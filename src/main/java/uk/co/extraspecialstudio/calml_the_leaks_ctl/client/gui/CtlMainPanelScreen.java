...
import uk.co.extraspecialstudio.calml_the_leaks_ctl.network.C2SWorldMemoryPacket;
...
public class CtlMainPanelScreen extends EscScreen {
    ...
    private static final String INTRO =
        "CTL reads AllTheLeaks output and adds plain-language context so you can spot real leaks versus normal noise. "
            + "Numbers update when you open this panel or press F8.";
        ...
        if (data.worldPersistenceAllowed) {
            ...
            if (data.worldPersistenceActive) {
                addButton(Component.literal("Stop saving & delete world memory"), worldBtn,
                    b -> CtlNetwork.CHANNEL.sendToServer(new C2SWorldMemoryPacket(C2SWorldMemoryPacket.ACTION_DISABLE_DELETE)));
            } else {
                addButton(Component.literal("Save leak history in this world"), worldBtn,
                    b -> CtlNetwork.CHANNEL.sendToServer(new C2SWorldMemoryPacket(C2SWorldMemoryPacket.ACTION_ENABLE)));
            }
            y += BUTTON_H + 4;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (activeTab == TAB_LEAKS && leakViewport != null && leakViewport.contains(mouseX, mouseY)) {
            leakScrollPx -= (int) (delta * LEAK_ROW_HEIGHT * 2);
            clampScroll();
            return true;
        }
        if (activeTab == TAB_GUIDE && guideViewport != null && guideViewport.contains(mouseX, mouseY)) {
            guideScrollPx -= (int) (delta * this.font.lineHeight * 3);
            clampScroll();
            return true;
        }
        return super.mouseScrolled(mouseX, mouseY, delta);
    ...
    private void drawWrapped(GuiGraphics g, Font font, String text, int x, int y, int maxWidth, int color) {
        EscText.drawWrapped(g, font, text, x, y, maxWidth, color);
    }
    ...
    private void renderGuideScrollable(GuiGraphics graphics) {
        ...
        EscText.drawString(graphics, f, "Understanding this panel", x, y, 0xFFDDAA, false, EscFonts.DEFAULT);
        y += lh;
        drawWrapped(graphics, f, INTRO, x, y, maxW, 0xCCCCCC);
        y += wrapHeight(f, INTRO, maxW);
        y += lh + GAP_AFTER_SECTION;

        EscText.drawString(graphics, f, "Server snapshot", x, y, 0xFFDDAA, false, EscFonts.DEFAULT);
        y += lh;
        String v1 = data.atlVersionLine != null ? data.atlVersionLine : "";
        String v2 = data.sparkVersionLine != null ? data.sparkVersionLine : "";
        ...
        EscText.drawString(graphics, f, "Spark, AllTheLeaks & CTL", x, y, 0xFFDDAA, false, EscFonts.DEFAULT);
        y += lh;
        ...
        if (data.worldPersistenceAllowed) {
            EscText.drawString(graphics, f, "World memory (optional)", x, y, 0xFFDDAA, false, EscFonts.DEFAULT);
            y += lh;
            if (data.worldPersistenceActive) {
                drawWrapped(graphics, f,
                    "World memory is ON. CTL stores leak history inside this world's save folder so it survives restarts. "
        ...
        EscText.drawString(graphics, f, "Narrow-down toolkit", x, y, 0xFFDDAA, false, EscFonts.DEFAULT);
        y += lh;
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        EscText.drawComponent(graphics, this.font, this.title, content.x(), 8, 0xFFFFFF, false);

        if (data == null) {
            EscText.drawString(graphics, this.font, "No diagnostics loaded. Run /ctl panel on the server.", content.x(), 32, 0xFFAA55, false, EscFonts.DEFAULT);
        ...
        if (activeTab == TAB_GUIDE && guideViewport != null && guideViewport.height() > 0) {
            ...
            if (tipLineArea != null && tipLineArea.height() > 0) {
                EscText.drawString(graphics, this.font, "Tip: refresh with F8 or /ctl panel.",
                    tipLineArea.x(), tipLineArea.y(), 0x888888, false, EscFonts.DEFAULT);
            }
            ...
            EscText.drawString(graphics, this.font, "All tracked leaks", leakHeaderArea.x(), leakHeaderArea.y(), 0xFFDDAA, false, EscFonts.DEFAULT);
            ...
            for (int i = 0; i < leaks.size(); i++) {
                ...
                if (rowY + LEAK_ROW_HEIGHT >= leakViewport.y() && rowY <= leakViewport.bottom()) {
                    ...
                    EscText.drawString(graphics, this.font, label, leakViewport.x() + 6, rowY + 6, hover ? 0xFFFFA0 : 0xFFFFFF, false, EscFonts.DEFAULT);
                }
            ...
            if (maxScroll > 0 && contentH > 0) {
                ...
                EscText.drawString(graphics, this.font, "No leaks yet — when AllTheLeaks reports one, it will show up here.",
                    leakViewport.x() + 4, leakViewport.y() + 8, 0x888888, false, EscFonts.DEFAULT);
            }
        }
