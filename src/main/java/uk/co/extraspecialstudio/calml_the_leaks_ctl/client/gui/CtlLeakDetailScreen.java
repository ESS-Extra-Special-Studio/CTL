...
public class CtlLeakDetailScreen extends EscScreen {
    ...
    private void renderTimeSeriesChart(GuiGraphics graphics) {
        ...
        int h = CHART_HEIGHT;
        EscText.drawString(graphics, font, "Count (recent samples)", left, top - 10, CtlGuiColors.text(0xAAAAAA), false, EscFonts.DEFAULT);
        ...
        EscText.drawString(graphics, font, "Bar color: heap % (red=high)", left, top + h + 2, CtlGuiColors.text(0x888888), false, EscFonts.DEFAULT);
    }
    ...
    private void renderSparkChartNote(GuiGraphics graphics) {
        ...
        int y = noteArea.y();
        EscText.drawString(graphics, font, "Spark vs this chart", x, y, CtlGuiColors.text(0xFFDDAA), false, EscFonts.DEFAULT);
        y += LINE_HEIGHT;
        for (FormattedCharSequence line : font.split(Component.literal(SPARK_CHART_NOTE).withStyle(s -> s.withFont(EscFonts.DEFAULT)), w)) {
            graphics.drawString(font, line, x, y, CtlGuiColors.text(0xAAAAAA), false);
            y += LINE_HEIGHT;
        }
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (lines.isEmpty()) {
            EscText.drawString(graphics, font, "No detail lines.", x, y, CtlGuiColors.text(0xAAAAAA), false, EscFonts.DEFAULT);
        } else {
            for (S2CPanelDataPacket.ColoredLine line : lines) {
