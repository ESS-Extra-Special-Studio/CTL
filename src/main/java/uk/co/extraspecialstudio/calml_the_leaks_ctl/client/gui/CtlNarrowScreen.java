...
public class CtlNarrowScreen extends EscScreen {
    ...
    private static final int COLOR_TITLE = CtlGuiColors.text(0xFFDDAA);
    private static final int COLOR_BODY = CtlGuiColors.text(0xCCCCCC);
    private static final int COLOR_MUTED = CtlGuiColors.text(0xAAAAAA);
    private static final int COLOR_SUMMARY = CtlGuiColors.text(0xB8E0FF);
    private static final int COLOR_WARN = CtlGuiColors.text(0xFFAA55);
    ...
    private void rebuildContent() {
        ...
        if (data == null) {
            appendWrapped(COLOR_WARN,
                "No diagnostics packet loaded. Open the main CTL panel first (ES Hub → UTILITY or /ctl panel), then open the narrow-down toolkit again.",
                w);
            return;
        }
        ...
        appendWrapped(COLOR_MUTED,
            "Bundled when the main diagnostics panel was last built. Run /ctl narrow compare, then refresh the main panel (ES Hub or /ctl panel) and re-open this toolkit to update.",
            w);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        scroll = (int) Math.max(0, Math.min(max, scroll - scrollY * LINE * 3));
        return true;
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        EscRect content = contentRect();
        EscText.drawComponent(graphics, font, title, content.x(), content.y() + 4, CtlGuiColors.text(0xFFFFFF), false);
