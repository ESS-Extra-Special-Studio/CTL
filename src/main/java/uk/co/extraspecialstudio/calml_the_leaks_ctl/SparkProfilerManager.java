...
public class SparkProfilerManager {
    ...
    private static boolean isSparkAlreadyRunning() {
        try {
            ...
            try {
                ...
                // Try Spark 2.x or other versions
            }
            
            // Try Spark plugin API
            try {
                Class<?> sparkPluginClass = Class.forName("me.lucko.spark.forge.plugin.ForgeSparkPlugin");
                Object pluginInstance = sparkPluginClass.getMethod("getInstance").invoke(null);
                Object profiler = sparkPluginClass.getMethod("getProfiler").invoke(pluginInstance);
                if (profiler != null) {
                    java.lang.reflect.Method isRunningMethod = profiler.getClass().getMethod("isRunning");
                    Boolean isRunning = (Boolean) isRunningMethod.invoke(profiler);
                    return isRunning != null && isRunning;
                }
            } catch (Exception e) {
                // Not available
    ...
    private static void triggerSparkViaReflection(MinecraftServer server) throws Exception {
        ...
        // Method 2: Try Spark's command handler directly (if available)
        try {
            Class<?> sparkPluginClass = Class.forName("me.lucko.spark.forge.plugin.ForgeSparkPlugin");
            Object pluginInstance = sparkPluginClass.getMethod("getInstance").invoke(null);
            
            // Try to get profiler from plugin
            Object profiler = sparkPluginClass.getMethod("getProfiler").invoke(pluginInstance);
            if (profiler != null) {
                ...
                profiler.getClass().getMethod("start").invoke(profiler);
                LOGGER.debug("[CTL] Triggered Spark profiling via reflection (Spark plugin API) - Spark may post to chat");
                return;
            }
        } catch (Exception e) {
            // Not available
